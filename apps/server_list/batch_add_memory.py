...
def search(ip, pattern):
    ...
    # res = cursor.fetchone()
    res = Pattern.objects.filter(pattern=pattern).values_list('ins_type', 'cpu_num', 'memory_num', 'flow_num',
                                                              'disk_num')
    ...
    if num == 0:
        ...
        c = int(ins_cpu / (all_cpu * num))
        m = int(ins_memory / (all_memory * num))
        f = int(ins_flow / (all_flow * num))
        d = int(ins_disk / (all_disk * num))
        return min(c, m, f, d)
