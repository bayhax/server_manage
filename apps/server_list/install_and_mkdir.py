...
def install(ip, user):
    ...
    stdin, stdout, stderr = ssh.exec_command(ssh_key_cmd)
    temp = stdout.read()
    ...
    if firewall_stat != "running":
        ...
        stdin, stdout, stderr = ssh.exec_command(start_firewall_cmd)
        temp = stdout.read()
    ...
    if stdout.read().decode('utf-8').strip() == '0':
        ...
        stdin, stdout, stderr = ssh.exec_command(swap_cmd)
        temp = stdout.read()
        # 设置永久挂载
...
if __name__ == "__main__":
    install(ip='172.24.0.16', user='root')
