...
def search(server_name, version, zone, plat, run_company, start, end):
    ...
    if version == "('')" or version == "()":
        sql = """select server_name,time from zero_break_log_search where (server_name = '%s' or '%s'='')
          ...
          and (time >= '%s' or '%s'='') and (time <='%s' or '%s'='');""" % (
            server_name, server_name, zone, zone, plat, plat, run_company, run_company, start, start, end, end)
    else:
        sql = """select server_name,time from zero_break_log_search where (server_name = '%s' or '%s'='')
